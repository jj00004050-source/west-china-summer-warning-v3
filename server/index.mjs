...
const sessionSecret =
  process.env.SESSION_SECRET || 'replace-this-secret-in-production'
...
const localVersion = data => {
  ...
  const stamp =
    ...
    `${pad(updated.getSeconds())}`

  const dates = [
    ...new Set(
      (data.batches || [])
        .flatMap(batch => batch.rows.map(row => row.targetDate))
        .filter(Boolean)
    )
  ].sort()
...
const withLocalVersion = data => ({
  ...empty,
  ...data,
  version: localVersion({ ...empty, ...data })
})
...
const readStore = async () => {
  try {
    return withLocalVersion(JSON.parse(await readFile(dataFile, 'utf8')))
  } catch {
    return withLocalVersion(empty)
  }
}
...
const writeBroadcast = data => {
  ...
  broadcastWriteQueue = broadcastWriteQueue.then(() =>
    writeFile(broadcastFile, JSON.stringify(next, null, 2))
  )
...
const needsHotelEnrichment = existing.hotels.some(
  hotel =>
    hotel.operationType == null || hotel.managementType == null
)
...
if (
  ...
  const defaultHotelByCode = new Map(
    defaults.hotels.map(hotel => [hotel.whCode, hotel])
  )
  ...
  const enrichedHotels = existing.hotels.length
    ? existing.hotels.map(hotel => {
        ...
        return {
          ...hotel,
          operationType:
            hotel.operationType || source?.operationType || '',
          managementType:
            hotel.managementType || source?.managementType || ''
        }
      })
    : defaults.hotels
  ...
  console.log(
    `基础数据初始化完成：${defaults.hotels.length} 家门店`
  )
}
...
const isAdmin = req => {
  const raw = cookie(req).west_admin
  if (!raw) return false

  const [role, expires, signature] = raw.split('.')
  const value = `${role}.${expires}`

  if (!signature || role !== 'admin' || Number(expires) < Date.now())
    return false
...
app.post('/api/admin/login', (_req, res) => {
  res.setHeader(
    'Set-Cookie',
    `west_admin=${token()}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800`
  )
  res.json({ ok: true })
})
...
app.post('/api/admin/logout', (_req, res) => {
  res.setHeader(
    'Set-Cookie',
    'west_admin=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0'
  )
  res.json({ ok: true })
})
...
app.put('/api/admin/data', requireAdmin, async (req, res) => {
  ...
  const versioned = withLocalVersion({
    ...next,
    version: undefined
  })

  await writeStore(versioned)

  res.json({
    ok: true,
    updatedAt: versioned.version.updatedAt,
    version: versioned.version
  })
})

app.get('/api/admin/broadcast', requireAdmin, async (_req, res) =>
...
app.listen(port, () =>
  console.log(`华西预警数据服务已启动：http://localhost:${port}`)
)
