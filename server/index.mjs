...
const sessionSecret = process.env.SESSION_SECRET || 'replace-this-secret-in-production'
const empty = { hotels: [], lastYear: [], renovations: [], batches: [], currentBaseDate: '', previousFinalSnapshot: undefined, mappings: {}, channelMappings: {}, qualityIssues: [], settings: { countMissingBookingAsZero: true } }
...
const token = () => { const value = `admin.${Date.now() + 8 * 3600000}`; return `${value}.${sign(value)}` }
...
const requireAdmin = (req, res, next) => isAdmin(req) ? next() : res.status(401).json({ error: '需要管理员登录' })
...
app.post('/api/admin/login', (req, res) => {
  ...
  res.setHeader('Set-Cookie', `west_admin=${token()}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800`)
  res.json({ ok: true })
})
app.post('/api/admin/logout', (_req, res) => {
  res.setHeader('Set-Cookie', 'west_admin=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0')
  res.json({ ok: true })
})
