...
import { adminLogout, adminSession, fetchData, saveServerData, type SaveProgress } from './utils/api'
...
export default function App() {
  ...
  useEffect(() => { reload(); if (isAdminPath) adminSession().then(setAuthenticated) }, [])
  ...
  if (isAdminPath) {
    ...
    return <div className="admin-shell"><header className="admin-top"><div><b>华西区域暑期预警 · 管理后台</b><span>{data.version?.source || '统一数据源'} · 写入及播报接口均已鉴权</span></div><nav><a href="/dashboard">打开只读看板</a><button onClick={async () => { await adminLogout(); setAuthenticated(false) }}>安全退出</button></nav></header><AdminDashboard data={data} onData={updateData}/></div>
  }
  ...
  const metricRows = buildMetricRows(data.hotels, filterBatch(selected), filterBatch(previous), data.lastYear, data.renovations || [])
  ...
  const rows = [...currentRowsForTarget, ...missingCurrentHotels.map(h => missingBookingMetricRow(h, targetDate, filters.dayOffset, data.renovations || []))]
  ...
  const diagnosticDays: DiagnosticDay[] = selectedDayTabs.flatMap(({ dayOffset, targetDate: dayTarget }) => {
    const dayRows = scoped.filter(row => row.targetDate === dayTarget)
    const dayCodes = new Set(dayRows.map(row => row.whCode))
    const completeRows = [...dayRows, ...activeScopedHotels.filter(hotel => !dayCodes.has(hotel.whCode)).map(hotel => missingBookingMetricRow(hotel, dayTarget, dayOffset, data.renovations || []))]
    const dayComparison = scopeComparisonRows(buildComparisonRows(data.hotels, data.lastYear, dayTarget).rows)
  ...
  const openingAgeOptions = [...new Set(visibleStoreRows.map(row => openingAgeOption(row.openDate, targetDate)))]
    .sort((a, b) => a === MISSING_OPENING_DATE ? -1 : b === MISSING_OPENING_DATE ? 1 : Number(a.replace('年', '')) - Number(b.replace('年', '')))
  const openingAgeFilteredStoreRows = openingAgeFilter.length
    ? visibleStoreRows.filter(row => openingAgeFilter.includes(openingAgeOption(row.openDate, targetDate)))
    : visibleStoreRows
  ...
  const viewContent = view === 'overview' ? <><div className="light-middle">{map}{channel}</div><RiskHeatmap days={diagnosticDays} level="province" title="省区 D0-D6 变化监控热力图" batch={selected?.batchTime || '--'} comparisonLabel={comparisonLabel} onSelect={(province, dayOffset) => drillProvince(province, dayOffset)}/><DistributionBoxPlot rows={rows} channelRows={channelRows} groupKey="province" title="省区门店分布箱形图" targetDate={targetDate} batch={selected?.batchTime || '--'} onGroup={province => drillProvince(province)} onStore={store => drillProvince(store.province)}/><ProvinceOverview rows={rows} comparisonRows={comparisonRows} channelRows={channelRows} countMissingAsZero={countMissingAsZero} onSelect={province => drillProvince(province)}/></>
    ...
    : view === 'area' ? <div className="view-stack">{areaBreadcrumb}<div className="diagnostic-tabs"><button className={areaTab === 'risk' ? 'active' : ''} onClick={() => setAreaTab('risk')}>风险概览</button><button className={areaTab === 'distribution' ? 'active' : ''} onClick={() => setAreaTab('distribution')}>门店分布</button><button className={areaTab === 'direct' ? 'active' : ''} onClick={() => setAreaTab('direct')}>直营店专项</button><button className={areaTab === 'detail' ? 'active' : ''} onClick={() => setAreaTab('detail')}>门店明细</button></div>
      {areaTab === 'risk' && <><RiskHeatmap days={diagnosticDays} level="revenueZone" title="商圈 D0-D6 变化监控热力图" batch={selected?.batchTime || '--'} comparisonLabel={comparisonLabel} onSelect={(zone, dayOffset) => drillZone(zone, dayOffset)}/><RankingChart rows={rows} level="revenueZone" title="收益管理商圈风险排名" onSelect={revenueZone => drillZone(revenueZone)}/></>}
    ...
    : <ChannelDrilldownView rows={channelRows} previousRows={previousChannelRows} stores={rows} comparisonLabel={comparisonLabel} settings={data.settings?.channelAnomaly} onStore={setSelectedStore}/>
  return <div className="light-app">
    <LightSidebar filters={filters} hotels={data.hotels} batches={data.batches} channels={channels} onChange={setFilters} onExport={exportView} view={view} onView={next => {
      ...
      setStoreTypeFilter('全部门店')
      setRenovationFilter('全部')
      setOpeningAgeFilter([])
