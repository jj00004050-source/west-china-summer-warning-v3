...
export default function StoreDetailDrawer({ store, allRows, comparisonRows, channelRows, comparisonLabel, priceSettings, onClose }: {
  ...
  const option = {
    ...
    legend: { data: ['理论RP', '同期RP'], textStyle: { color: '#71859a' } },
    ...
    series: [
      ...
      { name: '同期RP', type: 'line', smooth: true, data: mine.map(row => row?.lastRp), lineStyle: { color: '#9CA3AF', type: 'dashed' }, itemStyle: { color: '#9CA3AF' } },
    ],
  }
  return <div className="drawer-mask" onClick={onClose}><aside className="drawer store-anomaly-drawer" onClick={event => event.stopPropagation()}>
    ...
    <h3>当前数据</h3>
    <div className="drawer-kpis"><div><small>预订率 / 商圈</small><b>{fmtPct(store.bookingRate)}</b><em>{fmtPct(zone.bookingRate)} · {fmtPp(zoneGap)}</em></div><div><small>在手ADR</small><b>{fmtMoney(store.adr)}</b></div><div><small>理论RP / 同期RP</small><b>{fmtMoney(store.rp)}</b><em>{fmtMoney(store.lastRp)}</em></div><div><small>RP缺口</small><b className={(store.rpGap || 0) < 0 ? 'bad' : 'good'}>{fmtMoney(store.rpGap)}</b></div></div>
